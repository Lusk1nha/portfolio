...
interface TermLine { type: LineType; content: string }
...
function makeCTX() {
  ...
  const ctx: CommandContext = {
    ...
    navigate: (path) => { navigated.to = path },
    years: '4+ years',
  }
...
describe('terminal command: whoami', () => {
  it('outputs name and years', () => {
    ...
    whoami(ctx)
    expect(lines.some(l => l.content.includes('Lucas Pedro da Hora'))).toBe(true)
...
describe('terminal command: matrix (easter egg)', () => {
  it('outputs the Wake up line', () => {
    ...
    expect(lines.filter(l => l.type === 'accent').length).toBeGreaterThanOrEqual(2)
  })
})
...
describe('terminal command: fortune (easter egg)', () => {
  it('outputs a non-empty quoted string', () => {
    ...
    fortune(ctx)
    const quoteLine = lines.find(l => l.type === 'output' && l.content.includes('"'))
